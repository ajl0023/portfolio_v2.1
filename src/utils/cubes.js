...

{
	...
	const createCube = (size) => {
		...
		const speed = directions.reduce((object, axis) => {
			const max = size > sizes.m ? 0.3 : 0.6;
	...
	const getShading = (tint, rotate, distance) => {
		...
		if (rotate.x) darken.y = 0;
			...
			const { x } = distance;
			if (x > 90 && x < 270) directions.forEach((axis) => (darken[axis] = 1 - darken[axis]));
		}

		const alpha = (darken.x + darken.y) / 2;
		const blend = (value, index) =>
			Math.round(Strut.interpolate(value, tint.shading[index], alpha));
	...
	window.addEventListener('scroll', () => {
		...
		if (scroll < parallaxLimit) {
			...
			cubes.forEach(
				({ cube, speed }) =>
					(cube.style.transform = `translateY(${Math.abs(speed.x * 0.5) * scroll}px)`)
			);
			return;
		}
		headerIsHidden = true;
